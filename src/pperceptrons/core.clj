...
(extend-protocol PPperceptron
  ...
  (anneal-eta [pp] (update-in pp [:eta--learning-rate] (fn [x] (* x 0.999)) ))  ;;WIP there is a specific algo for this...
  )
...
(defn make-resonable-pp
 ([inputsize  epsilon zerod?  seed] (make-resonable-pp inputsize epsilon zerod? seed 1))
 (                    [inputsize  ;;do a (count input)
                       ...
                       size-boost] ;;How many times should the pp be bigger then recomended
...
(let [pp (make-resonable-pp 2 0.75 false 42 1)]
  (map  (fn [x] (read-out pp [-0.9 x])) (range -3 3 0.01)))
